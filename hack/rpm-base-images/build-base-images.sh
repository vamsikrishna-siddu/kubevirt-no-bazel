...
#
# The RPM trees are generated with standalone bazeldnf (no Bazel needed).
...
# Generate the tars that Containerfile.<image> copies into the directory
# given as $3, which is then used as the build context.
generate_tars_for_image() {
    local image=$1
    local arch_tag=$2
    local context=$3
    local rpm_arch
    ...
    for tree in ${trees}; do
        generate_rpm_tar "${tree}_${rpm_arch}_cs${CENTOS_STREAM_VERSION}" \
            "${context}/${tree}_${rpm_arch}_cs${CENTOS_STREAM_VERSION}.tar"
    done
    if [[ "${image}" == "libguestfs-tools" ]]; then
        generate_appliance_tar "${rpm_arch}" "${context}/appliance_layer_${rpm_arch}.tar"
    fi
}
...
build_for_arch() {
    ...
    for image in "${arch_images[@]}"; do
        ...
        echo ""
        context="${RPM_TARS_DIR}/${image}-${arch_tag}"
        generate_tars_for_image "${image}" "${arch_tag}" "${context}"
